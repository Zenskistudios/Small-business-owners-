...
function InvoiceTab() {
  ...
  const total = items.reduce((sum, it) => sum + (parseFloat(it.price) || 0) * (parseInt(it.qty) || 0), 0);
  ...
  return (
    <div>
      ...
      <div className="section-sub">Create professional invoices in seconds. Share or download as PDF.</div>
...
function BookingTab() {
  ...
  return (
    <div>
      ...
      <div className="section-sub">Set up your availability and share a booking link with clients.</div>
      ...
      {selected && (
        <div className="card" style={{borderColor: "#f5a623"}}>
          ...
          <div className="grid-2">
            <div><label>Your Name</label><input value={clientName} onChange={e => setClientName(e.target.value)} placeholder="Jane Doe" /></div>
          ...
          <div className="flex-end">
            ...
            <button className="btn btn-primary" onClick={confirm} disabled={!clientName}>Confirm Booking</button>
          </div>
        </div>
      )}
...
{
  "posts": [
    { "platform": "Instagram", "content": "...", "hashtags": ["tag1","tag2","tag3"] }
  ]
}
One post per platform. Make each post optimized for that platform's style and character limits.`;
    ...
    try {
      ...
      setResults([{ platform: "Error", content: "Failed to generate. Please try again.", hashtags: [] }]);
    }
    setLoading(false);
  };
  ...
  return (
    <div>
      ...
      <div className="section-sub">Generate platform-optimized posts for your business in seconds.</div>
      ...
      {loading && (
        <div className="card">
          <div className={`ai-output loading`}><span className="typing-dots">Creating your posts</span></div>
        </div>
      )}
